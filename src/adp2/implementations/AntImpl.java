...
public class AntImpl implements Ant {
	...
	private Set<Integer> unvisitedNodes; // alle Knoten des Graphen, bei dem
											// Ameise noch nicht war
	...
	/*
	 * bildet zur aktuellen Position alle unbesuchten Nachbarn dieses Knotens
	 * auf einen Balance-Wert ab
	 * 
	 * Formel zur Berechnung der Balance: [alpha * (Pheromon/maxPheromon) + [(1-
	 * alpha) * (Distanz/maxDistanz)] dabei geben Pheromon und Distanz die
	 * jeweiligen Werte der aktuellen Kante zurueck die MaxWerte die maximalen
	 * Werte aller inzidenten Kanten des aktuellen Knotens
	 */
	private Map<Integer, Double> balances() {
		...
		for (Integer elem : this.unvisitedNeighbors()) {
			result.put(
					elem,
					balance1(maxDist, maxPher, g.intensity(position(), elem),
							g.distance(position(), elem)));
		}
		return result;
	...
	private double balance1(double maxDist, double maxPher, double pher,
			double dist) {
		double distanz;
		if (!(maxDist == 0)) {
		...
		return balance * 1000 + 1; // balance * 1000, damit die +1 nicht stark
									// ins Gewicht faellt
	}

	// Bestimmt die Balance mit alpha * pheromon + (1-aplha) * (maxDist
	// -distanz)
	private double balance2(double maxDist, double maxPher, double pher,
			double dist) {
		double distanz;
		if (!(maxDist == 0)) {
	...
	public void step() {
		...
		if (waitingTime > 0)
			...
			if (probability.isEmpty()) {
				...
				/*
				 * bildet die Summe aus allen Balancewerten ==> aus den
				 * einzelnen Werten und der Summe wird im naechsten Schritt die
				 * Wahrscheinlichkeit gebildet
				 */
				double summe = sumOfValues(probability);

				/*
				 * bildet die Nachbarn auf einen Wert zwischen 0 und 1 ab Wert =
				 * eigene Wahrscheinlichkeit + vorher berechneter Wert Bsp:
				 * 
				 * Node Wahrscheinlichkeit Wert 1 0.3 0.3 2 0.1 0.4 3 0.5 0.9 4
				 * 0.1 1.0
				 */
				double vorgaenger = 0;

				for (Map.Entry<Integer, Double> entry : probability.entrySet()) {
					double wahrscheinlichkeit = entry.getValue() / summe
							+ vorgaenger;
					if (wahrscheinlichkeit > 1.0)
				...
				/*
				 * Integriert die zufaellig Auswahl des Weges der Ameise durch
				 * verrechnen eines Random Wertes (0.0 - 1.0) mit der vorher
				 * errechneten Wahrscheinlichkeit ==> Wahl des naechsten Punktes
				 * auf dem Weg
				 * 
				 * 
				 * minNode evtl auf -1 setzen ==> fehlerbehandlung danach
				 * moeglich
				 */
				...
				if (unvisitedNodes.isEmpty()
						&& path.get(path.size() - 1) == path.get(0)) {
					finished = true;
					System.out.println(this + " fertig");
				}
			...
			// waitingTime=g.distance(path.get(path.size()-2),
